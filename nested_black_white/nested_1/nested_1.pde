/**
 ...
 这里可以调整的参数比较多
 调整不同的格子尺寸，最大数位数，相邻两列距离，画布尺寸以及画哪几个数位可查看不同图形
...
int d = 0;
int b = 0;
int maxDigits = 2;  // 最大数位数
int distance = 2;  // 相邻两列之间距离
int base;  // 进制
int nth = 1;  // 查看特定数位用。画出第nth位到第nth + min(maxDigits, distance) - 1位。1指最低位。不足的不画。
int offset = 48002;  // 数值从这里开始一帧加1。不小于2以免画不出预期图形。可以看后面的。
...
void setup() {
  ...
  colorArray = new int[Integer.toString(n1, 2).length()];
...
void draw() {
  ...
  if (base < colorString.length()) {
    colorArray = new int[colorString.length()];
  }
  ...
  frameName = String.format("黑和白的舞蹈_嵌套_%dx%d格_格子尺寸%dx%d_数位范围%d到%d_数字范围%d到%d_0的颜色%d_1的颜色%d_背景颜色%d.png", width/cellWidth, height/cellHeight, cellWidth, cellHeight, nth, nth + min(maxDigits, distance) - 1, n0, frameCount + offset - 1, color_0, color_1, backgroundColor);
