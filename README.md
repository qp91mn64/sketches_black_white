...
- multiple_3.pde：一次函数 $ pn+a_0 $ 加上第三进制
- quadratic_1.pde：二次方
...
multiple_1.pde：一次加p，也可以认为是p的整数倍取二进制。只需考虑p是奇数的情形  
multiple_2.pde：加常数项 $a_0$ ，只需考虑 $a_0$ 小于p的情形
multiple_3.pde：运算结果按照第三进制的方式画出  
...
对应power_1.pde  
power_2.pde乘以系数 $a$  
power_3.pde幂函数的底换成一次函数 $a_1n+a_0$
...
nested_2.pde：与nested_1.pde的不同之处是  
再转换进制时  
进制与第一次转换得到的位数无关  
便于画特定进制对应图形  
